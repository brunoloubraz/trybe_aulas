...
//         return console.log(`Destinatário: ${clients[index].name}. Endereço: ${clients[index].address.street}, ${clients[index].address.number}, ${clients[index].address.neighborhood}, ${clients[index].address.city}, ${clients[index].address.state}, ${clients[index].address.cep}`);
//       }
//     }
//   } catch (error) {
...
const findPersonByPosition = (position) => {
  ...
  try {
    ...
    return `Cliente ${clients[id].name} Email: ${clients[id].email}`
  } catch (error) {
    return error.message
  }
};
...
const findPeopleByState = (state) => {
  ...
  try {
    ...
    if (array.length === 0) {
      throw new Error('Algo de errado aqui')
    }
      return array
  } catch (error) {
    return error.message
